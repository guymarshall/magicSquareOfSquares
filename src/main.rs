...
const fn sums_are_equal(numbers: &[u32; 9]) -> bool {
    ...
    let left_column_sum: u32 = numbers[0] + numbers[3] + numbers[6];
    let middle_column_sum: u32 = numbers[1] + numbers[4] + numbers[7];
    let right_column_sum: u32 = numbers[2] + numbers[5] + numbers[8];

    if bottom_row_sum != left_column_sum || left_column_sum != middle_column_sum || middle_column_sum != right_column_sum {
        return false;
    }

    let nw_se_sum: u32 = numbers[0] + numbers[4] + numbers[8];
    let sw_ne_sum: u32 = numbers[6] + numbers[4] + numbers[2];
